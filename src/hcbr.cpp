...
int main(int argc, char** argv)
{
    ...
    const auto input_source =  params["input"]["source"];
    ...
    */
    ...
    if(not deserialize) {
        ...
        else if (input_source == "stdin") {
            cerr << "# Read cases from stdin..." << endl;
            for (std::string line; std::getline(std::cin, line);) {
                if (!line.empty()) {
                    nc = line_to_vect<int>(line);
                    o = nc.back();
                    nc.pop_back();
                    cb.add_case(nc, o, false);
                } else {
                    break;
                }
            }
    ...
    for(auto iter = -1; iter < max_learning_iterations + 1; ++iter) 
    {
        ...
        fn = 0;
        log_training = "";
        for(auto i = 0; i < size(cb.cases); ++i) 
        {
            o = cb.outcomes[i];
            nc = cb.cases[i];
            ...
            pred = normalize_prediction(pred_0, pred_1, 0, 0, 0);
            prediction = prediction_rule(pred, rdf, 0, 0, 0, 0, 0, 0, 1, 0, gen);

            if(iter >= max_learning_iterations)
                pred_outfile << std::setprecision(15) << i << " " << o << " " <<  prediction << " " << pred_1 << " " << pred_0 << std::endl;
                ...
                if(abs(o - prediction) != 0) {
                    for(const auto& k: proj.first) {
                        if(prediction == 1) {
                            ...
                            cb.e_intrinsic_strength[0][k.first] += r * err;
                            cb.e_intrinsic_strength[1][k.first] -= r * err;
                        } else {
    ...
    if(!no_pred) {
        ...
        log.open(log_file_name("prediction", run_id), std::fstream::in | std::fstream::out | std::fstream::app);
        start_time = std::chrono::steady_clock::now();
        for(auto i = limit_examples; i < n_cases; ++i) {
            log_prediction = "";
