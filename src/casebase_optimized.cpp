...
public:
    ...
    void add_case(std::vector<int> new_case, int outcome) {
        ...
        // TODO: Parallelize it. Guard on intersecting_ei. Check if performance gain.
        for(auto f: new_case) {
            if(f_to_e.count(f) == 1) {
        ...
        for(auto e: intersection_map) {
            if(std::size(intersection_family[e.first]) == std::size(e.second)) {
                ...
                // TODO: Parallelize it. To check before if feasible.
                for(auto f: e.second) {
        ...
        for (auto e: c_to_e[case_index]) {
            raw_c_to_e_overlap[case_index][e] = case_overlap(new_case, intersection_family[e]);
        }

        // TODO: Parallelize it. Easy.
        for(auto e: to_update) {
            raw_c_to_e_overlap[e.first][e.second] = case_overlap(cases[e.first], intersection_family[e.second]);
        }

        // TODO: Parallelize it. Probably easy.
        for (auto e: c_to_e[case_index])
        {
        ...
        std::set<int> e_to_update;
        for (auto e: intersection_map) {
            for(auto c: e_to_c[e.first]) {
                for(auto e2: c_to_e[c]) {
                    e_to_update.insert(e2);
                }
            }
        }
        
        for(auto e: e_to_update) {
            _non_normalized_intrinsic_strength(0, e);
            _non_normalized_intrinsic_strength(1, e);
        }
    ...
    double mu(int o, int ei, int c) {
        auto total = double{0};
        double top = e_to_outcome_count[ei][o] * raw_c_to_e_overlap[c][ei]; //case_overlap(case_details, ei_details);

        // TODO: Parallelize it. Easy.
        for(auto e: c_to_e[c]) {
            total += e_to_outcome_count[e][o] * raw_c_to_e_overlap[c][e]; //case_overlap(case_details, intersection_family[e]);
        }
        if(total == 0) {
            return 0.;
        } else {
    ...
    void calculate_intrinsic_strength(int o, int ei) {
        ...
        auto ei_strength = e_non_normalized_intrinsic_strength[o][ei];
        // TODO: Parallelize it. Easy.
        for(int i=0; i < std::size(intersection_family); ++i) {
            all_strength += e_non_normalized_intrinsic_strength[o][i];
        }
        /*
        double all_strength = std::experimental::parallel::transform_reduce(
            std::experimental::parallel::par,
            std::begin(intersection_family), std::end(intersection_family),
            0.0,
            [o](auto i) { return _non_normalized_intrinsic_strength(o, i); }
        );*/
        if(all_strength > 0) {
    ...
    std::map<int, std::map<int, double>> e_non_normalized_intrinsic_strength; ///< Non Normalized Intrinsic strength of intersecting elements
    std::map<int, std::map<int, std::map<int, double>>> c_to_e_overlap;       ///< Overlapping value between cases and intersecting elements
    std::map<int, std::map<int, double>> raw_c_to_e_overlap;                 ///< 
...
private:
    ...
    double _non_normalized_intrinsic_strength(int o, int ei) {
        ...
        auto top = double{0.};
        // TODO: Parallelize it. Easy.
        for(auto i = 0; i < std::size(ca); ++i) {
            top += c_to_e_overlap[o][ca[i]][ei];
        }
        e_non_normalized_intrinsic_strength[o][ei] = top * res;
        return top * res;
    }
