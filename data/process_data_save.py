...
def main():
    ...
    for k, v in enumerate(mapping.values()):
        # Justice mapping
        #print(k)
        for c in v:
            i = justices_map[c[justice_column].upper()]
            ...
            justice_per_case[k].extend([
                final_justices[i][1],
                final_justices[i][2]
                ])
    
    cases = [c[0] for k, c in mapping.iteritems()]

    final_cases = filter_cases(cases, except_features_no_outcomes, max_feature_for_justice + 1)
